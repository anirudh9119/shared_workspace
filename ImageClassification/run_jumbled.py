...
parser.add_argument('--regressive', type = str2bool, default = False)
parser.add_argument('--attn_iters', type = int, default = 1)
parser.add_argument('--slot_attention', type = str2bool, default = False)
...
if args.shared_memory_attention:
    if not args.slot_attention:
        rmc_params = sum(p.numel() for p in net.net.enc.self_attn.relational_memory.parameters() if p.requires_grad)
        for p in net.net.enc.self_attn.relational_memory.parameters():
             if p.requires_grad:
                 print(p.shape)
        logging.info("Total number of rmc_parameters:{}".format(rmc_params))
        print("Total number of rmc_parameters:{}".format(rmc_params))
...
def patch_shuffle(permutation,img):
    tmp = img.clone()
    for i in range(64):
        l = permutation[i]//8
        r = permutation[i]%8
...
def train(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        x_stack = patch_shuffle(_shf,inputs)
        imgs = x_stack
